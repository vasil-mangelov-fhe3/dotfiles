...
function __promptline_ps1 {
  ...
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "b" slices
  __promptline_wrapper "$([[ -n ${SSH_AUTH_SOCK} ]] && if [[ -n ${ZSH_VERSION-} ]]; then print "%n@$(__promptline_host)"; elif [[ -n ${FISH_VERSION-} ]]; then printf "%s" "$USER@$(__promptline_host)"; else printf "%s" "\\u@$(__promptline_host)"; fi )" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
